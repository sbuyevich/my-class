...
To use a different class code or port, open a terminal in this folder and run:

```bat
.\scripts\run.bat demo 5555
```
...
The launcher opens the correct teacher-computer URL automatically. Students must be on the same Wi-Fi/LAN and use the same class code.
...
- `timeLimitSeconds`: optional timer just for this question. If omitted, the app uses `quiz.json` `timeLimitSeconds`.
- `answerCount`: optional number of answer buttons just for this question. If omitted, the app uses the quiz default.
